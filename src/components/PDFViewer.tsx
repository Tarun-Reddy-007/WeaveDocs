...
export function PDFViewer({
  ...
  useEffect(() => {
    ...
    return () => { ro.disconnect(); window.removeEventListener('resize', updateContainerSize); };
  }, []);
  ...
  const renderWidth = useMemo(() => {
    ...
    const scale = Math.min(containerSize.width / pageSize.width, containerSize.height / pageSize.height);
  ...
  const handlePageLoadSuccess = (page: {
    ...
    setPageSize({ width: page.originalWidth ?? page.width, height: page.originalHeight ?? page.height });
    setIsLoading(false);
  };
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full bg-transparent p-4 flex ${
        fitToContainer
    ...
    >
      ...
      {isLoading && (
        <div className="flex items-center gap-3 text-[11px] tracking-[0.2em] uppercase text-gray-400 font-['DM_Sans',sans-serif] m-auto">
          <span className="w-4 h-4 border border-gray-300 border-t-black animate-spin" />
          Loading
        </div>
      )}
      ...
      {pdfUrl && containerSize.height > 0 && !error && (
        ...
        >
          {/* Page wrapper — subtle shadow to lift off bg */}
...
export function PDFThumbnail({
  ...
  return (
    <div
      ...
      className={`w-full flex flex-col cursor-pointer transition-all duration-150 font-['DM_Sans',sans-serif] group`}
    >
      {/* PDF canvas area */}
      <div
        ref={containerRef}
        className={`w-full overflow-hidden flex items-center justify-center transition-all duration-150 ${
          isActive
            ? 'ring-[1.5px] ring-gray-400 shadow-[0_2px_8px_0_rgba(0,0,0,0.08)]'
      ...
      {/* Title input — only shown when editing, otherwise plain text */}
